...
import { UsersIcon, EyeOffIcon } from "@heroicons/react/solid";
import axios from "axios";
import { toast } from "react-toastify";
import { useRouter } from "next/router";
import ConfirmModal from "../Modals/ConfirmModal";

const PublicCard = (tournament) => {
  ...
  const handlePrivate = async () => {
    await axios.put(
      `${process.env.API_URL}/tournament/privateTournament/${tournament.id}`
    );
    toast.success("Tournament made private");
    router.replace(router.asPath);
  };

  let action = handlePrivate;
    ...
    <div className="flex py-7 px-2 border-b first:border-t">
      ...
      <div className="flex flex-col flex-grow pl-5">
        <div className="flex justify-end">
          <EyeOffIcon
            onClick={() => {
              setTitle("Make tournament private?");
              action = handlePrivate;
